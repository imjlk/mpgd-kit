import { aitGameBundleBasePath, rewriteAitGameBundleUrl } from './wrapper-url';

const gameBundleBasePath = aitGameBundleBasePath;
...
export function mountAitGameBundle(app: HTMLElement): Promise<void> {
  return loadGameBundle(app);
}

async function loadGameBundle(app: HTMLElement): Promise<void> {
  try {
...
async function fetchGameBundleDocument(): Promise<Document> {
  ...
  try {
    ...
    // The wrapper only runs over HTTP(S), where Vite and the AIT container both declare HTML.
    // Missing metadata fails closed before any URLs from the response are injected.
...
async function appendBundleLink(source: HTMLLinkElement): Promise<void> {
  ...
  copyAttribute(source, link, 'type');
  link.rel = rel;
...
async function appendBundleScripts(scripts: readonly HTMLScriptElement[]): Promise<void> {
  ...
  try {
    for (const script of scripts) {
      const scriptPromise = appendBundleScript(script);
      // The runtime trap can win while this resource is still loading. Mark its eventual
      // rejection as handled without changing which error wins the race.
      void scriptPromise.catch(() => undefined);
      await Promise.race([scriptPromise, runtimeErrorTrap.promise]);
...
function rewriteGameBundleUrl(url: string): string {
  return rewriteAitGameBundleUrl(url, window.location.origin);
}
...
function renderMissingBundleShell(app: HTMLElement, error: unknown): void {
