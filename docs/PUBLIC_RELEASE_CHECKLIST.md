...
- [ ] Deploy the exact preflighted Microsoft Store PWA and manifest icons, then run `pnpm exec mpgd target generate-package microsoft-store --targets-file examples/phaser-starter/mpgd.targets.json --pwa-url <public-https-url> --manifest-url <public-manifest-url> --version <modern-version> --classic-version <lower-classic-version>`. Confirm the command verifies the deployed manifest and every icon before and after generation, preserve the generated provenance evidence, inspect the downloaded ZIP with the package acceptance flow, and only then upload accepted packages to Partner Center.
