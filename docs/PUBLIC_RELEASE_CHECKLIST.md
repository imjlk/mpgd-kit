...
- [ ] Copy the PWABuilder packages under `examples/phaser-starter/release-input/microsoft-store`; then, on Windows in an active administrator user session, run `pnpm exec mpgd target accept-package microsoft-store --targets-file examples/phaser-starter/mpgd.targets.json --packages <game-relative package paths>` and attach `examples/phaser-starter/release-output/microsoft-store/package-acceptance.json` plus the WACK reports.
