...
const integrity = { bytes: 3, sha256: createHash('sha256').update(bytes).digest('hex') };
const options = () => ({
...
it('preserves read errors and releases the reader lock when cancelling an errored stream fails', async () => {
  ...
  const reader = { read: vi.fn().mockRejectedValue(failure), cancel: vi.fn().mockRejectedValue(new Error('cancel failed')), releaseLock };
...
it('retains retry classification if discarding an HTTP error body rejects', async () => {
  const fetch = vi.fn().mockResolvedValueOnce({ ok: false, status: 500, body: { cancel: vi.fn().mockRejectedValue(new Error('broken body')) } }).mockResolvedValueOnce(new Response(bytes));
  vi.stubGlobal('fetch', fetch);
  expect(new Uint8Array(await (await fetchPackFile('/image.png', options())).arrayBuffer())).toEqual(
    bytes,
  );
...
it.each([
  ...
  [new Uint8Array([1, 2, 3, 4]), 'byte limit'],
...
it('caps streaming bodies even without integrity metadata', async () => {
  ...
  await expect(fetchPackFile('/image.png', { signal: new AbortController().signal, retries: 0, maxFileBytes: 8 })).rejects.toThrow('byte limit');
});
