...
export function materializeCapacitorShellStarter(
  ...
  // workspace globs, so install it standalone only when its CLI is absent.
  // Always retry installs, including after a previous install was interrupted.
  runner.run('pnpm', ['install', '--no-frozen-lockfile'], plan.gameRoot);
  if (!existsSync(path.join(plan.gameRoot, shellPath, 'node_modules/.bin/cap'))) {
    runner.run(
      'pnpm',
