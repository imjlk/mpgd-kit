...
const nativePlatform = target.kind === 'capacitor-android'
  ? 'android'
  : target.kind === 'capacitor-ios'
    ? 'ios'
    : undefined;
const nativeAttempt = nativePlatform === undefined
  ? undefined
  : beginNativeBuildAttempt(configBaseDir, targetName);
const nativePlan = nativePlatform === undefined
  ? undefined
...
const nativeWebBundle = nativePlan === undefined
  ? undefined
  : mkdtempSync(join(tmpdir(), 'mpgd-native-web-'));
const nativeArtifactRoot = nativeAttempt === undefined
  ? undefined
  : `release-output/native/${targetName}/${safeArtifactFileStem(profile)}/${nativeAttempt.runId}`;
...
const env: NodeJS.ProcessEnv = {
  ...
  MPGD_EFFECTIVE_TARGET_CONFIG_OUTPUT_DIR: nativeAttempt === undefined
    ? effectiveTargetConfigOutputDir(configBaseDir)
    : targetPath(`artifacts/target-config/native/${targetName}/${nativeAttempt.runId}`),
...
try {
  ...
  if (target.kind !== 'devvit-web' && !isMiniGameTarget(target)) {
    const webOutput = nativeWebBundle ?? `${gameApp}/dist`;
    run(
      'pnpm',
      [
        '--dir',
        ...
        '--mode',
        profile,
        ...(nativeWebBundle === undefined ? [] : ['--outDir', nativeWebBundle, '--emptyOutDir']),
      ],
      env,
    );
  ...
  if (nativeWebBundle !== undefined) {
    rmSync(nativeWebBundle, { force: true, recursive: true });
  }
